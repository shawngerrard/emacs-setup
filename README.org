...
~lsp~ is a standardized protocol that acts as an interpreter in an IDE and communicates with language servers to provide "language intelligence tools", such as intelli-sense. You need to install LSP into DOOM as well as a language server for the particular language you want to provide support for.
...
You can install a language server in DOOM using ~M-x lsp-install-server~.

I use ~ts-ls~ (Typescript Language Server) for supporting ~ts~ code.
