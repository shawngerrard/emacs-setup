...
(use-package org-roam
    :ensure t
    :custom
    (org-roam-directory (file-truename "~/documents/second-brain/"))
...
#+begin_quote
The above code will set the scanning roam directory for ~.org~ files to ~​~/documents/second-brain/~, bind keys to popular roam functions, set a basic roam node template configuration, and ensure the ~sqlite3~ database is synced with your ~.org~ files in the previously-mentioned directory.
#+end_quote
